        ...
        <div class="card-body">
            ...
            <div class="ms-4">
                ...
                <table>
                    ...
                    <tr>
                        <td>Rating Usia : </td>
                        <td style="width: 50px"></td>
                        <td><b><span class="badge badge-danger">{{ $ticket['schedule']['movie']['age_rating']}}</span></b></td>
                    </tr>
                </table>
            </div>
            ...
            <table>
                <tr>
                    <td>Kursi Dipilih :</td>
                    <td style="width: 50px"></td>
                    <td><b>{{ implode(',', $ticket['rows_of_seats'])}}</b></td> {{-- Implode: array jadi sting dipisahkan dengan tada ','--}}
                </tr>
                <tr>
                    <td>Harga Tiket :</td>
                    <td style="width: 50px"></td>
                    <td>Rp. <b>{{number_format($ticket['schedule']['price'], 0, ',', '.') }}</b>
                    <b class="text-secondary"> x {{$ticket['quantity'] }}</b></td>
                </tr>
                 <tr>
                    <td>Biaya Layanan :</td>
                    <td style="width: 50px"></td>
            ...
            <select name="promo_id" id="promo_id" class="form-select" onchange="selectPromo(this)">
                ...
                <option value="{{ $promo['id'] }}">{{ $promo['promo_code'] }} - {{ $promo['type'] ==
                 'percent' ? $promo['discount'] .'%' : 'Rp.' . number_format($promo['discount'], 0, ',', '.
                 ')  }}</option>
                 @endforeach
                @endif
...
<div class="fixed-bottom w-100 p-2 text-center" style="background: #112646; color:white; cursor: pointer" onclick="createQR()"><b>BAYAR SEKARANG </b></div>
@endsection
